...
async def start_handler(bot: Client, m: Message):
    ...
    await m.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("Developer - The_Dsr", url="https://t.me/The_dsr")],
                [InlineKeyboardButton("Support Group", url="https://t.me/Dsrs_Group"),
                 InlineKeyboardButton("Bots Channel", url="https://t.me/kashir_bots")],
...
async def callback_handlers(bot: Client, cb: CallbackQuery):
    if "mergeNow" in cb.data:
        ...
        if (QueueDB.get(cb.from_user.id, None) is None) or (QueueDB.get(cb.from_user.id) == []):
            await cb.answer("Sorry Unkil, Your Queue is Empty!", show_alert=True)
            return
        merged_vid_path = f"{Config.DOWN_PATH}/{str(cb.from_user.id)}/[@Akashir_bots]_Merged.{FormtDB.get(cb.from_user.id).lower()}"
